...
const getAndDrawUsers = async () => {
    await fetch("http://172.20.10.7:8192/api/filters/get-all")
        .then(res => res.json())
        .then(data => {
            if (data.data !== null) {
...
const submitFilters = async (data) => {
    await fetch("http://172.20.10.7:8192/api/filters/edit-position", {
        method: "PUT",
        headers: {
        ...
        usersList.insertAdjacentHTML("beforeend", `
                <li id="filter-${filter.id}" class="users__item users-item">
                    <div class="users-item__column users-item__name">
                        <div style="cursor: default" class="users-item__link">${filter.name.toUpperCase()}</div>
                    </div>
